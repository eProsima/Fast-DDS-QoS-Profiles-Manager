...
int set_verb(cxxopts::Options global_parser, int argc, char** argv){
	...
	if (3 == argc || 5 == argc)
	{
		...
		else std::cout << "No \"where\" parsed" << std::endl;
		...
		else std::cout << "No \"what\" parsed" << std::endl;
...
int main(int argc, char** argv){
	...
	// First level parser
	cxxopts::Options global_parser("poc_cxxopts", "PoC for cxxopts argument parsing library for Fast DDS QoS Profiles Manager");
	...
	// Global help print
	if (result["file"].as<std::string>() == "help" || result.count("verb") == 0 || result["verb"].as<std::string>() == "help")
    {
        std::cout << global_parser.help({ "global", "" }) << std::endl;
